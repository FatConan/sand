...
class RE(RenderEntity):
    pass

def test_render_entity():
    re = RE(test_data["site"], test_data["target"], test_data["source"])
...
def test_render_entity_validation():
    ...
    re = RE(test_data["site"], test_data["target"], None)
    assert re.validate() == True
    ...
    re = RE(test_data["site"], None, test_data["source"])
